...
module.exports = function (expect) {
    return expect.addAssertion('to be middleware that processes', function (expect, subject, value, done) {
        ...
        var requestProperties = _.extend({}, value.request),
        ...
        if (requestProperties.body) {
            if (requestProperties.body.pipe) {
                requestStream = requestProperties.body;
                ...
                if (typeof requestProperties.body === 'object') {
        ...
        ['write', 'end', 'destroy'].forEach(function (methodName) {
            ...
            res[methodName] = function (chunk, encoding) {
                ...
                if (methodName === 'end' || methodName === 'destroy') {
                    ...
                    var formattedExpectedResponseHeaders = formatHeaderNames(expectedResponseProperties.headers);
                    ...
                    if (typeof expectedResponseProperties.body === 'string') {
                        ...
                        formattedExpectedResponseHeaders['Content-Type'] = formattedExpectedResponseHeaders['Content-Type'] || 'application/json; charset=utf-8';
                    }
                    expect(formatHeaderNames(res._headers), 'to have properties', formattedExpectedResponseHeaders);
                    try {
