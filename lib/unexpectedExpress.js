        ...
        function doTheAssertions(errorPassedToNext) {
            ...
            try {
                ...
                if (!httpResponse.satisfies(expectedResponseProperties)) {
                    var expectedStatusLine = new messy.StatusLine(_.defaults(_.pick(expectedResponseProperties, messy.StatusLine.propertyNames), httpResponse.statusLine))
                        expectedHeaders = new messy.Headers(expectedResponseProperties.headers),
                        bodyMatches = !('body' in expectedResponseProperties) || httpResponse.satisfies({
                            body: expectedResponseProperties.body
                        });
                    _.defaults(expectedHeaders.valuesByName, httpResponse.headers.valuesByName);
                    expect.fail(function (output) {
                        if (httpResponse.statusLine.satisfies(expectedStatusLine)) {
                            output.append(expect.inspect(httpResponse.statusLine));
                        } else {
                            output.append(expect.diff(expectedStatusLine.toString(), httpResponse.statusLine.toString()).diff);
                        }
                        output.nl();
                        if (httpResponse.headers.satisfy(expectedResponseProperties.headers)) {
                            output.append(expect.inspect(httpResponse.headers));
                        } else {
                            output.append(expect.diff(
                                expectedHeaders.toString().replace(/\r\n$/, '').split(/\r\n/).sort().join('\n'),
                        ...
                        if (bodyMatches) {
                            ...
                            output.append(expect.diff(expectedResponseProperties.body, httpResponse.body).diff);
                        }
                        return output;
                   });
                }
