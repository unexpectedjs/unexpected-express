  ...
  installInto(expect) {
    ...
    expect.exportAssertion(
      ...
      (expect, subject, value) => {
        ...
        const missingProperties = Object.keys(value).filter(key => key !== 'request' && key !== 'response');
        ...
        let requestForm = requestProperties.form;
        ...
        const expectedMetadata = _.extend(
          {},
        ...
        return expect
          ...
          .then(() => {
            ...
            const promiseByKey = {
              httpExchange: expect.promise(() => expect(context.httpExchange, 'to satisfy', {
                response: expectedResponseProperties
              })),
              metadata: {}
            };
            Object.keys(expectedMetadata).forEach(key => {
              promiseByKey.metadata[key] = expect.promise(() => topLevelExpect(
            ...
            return expect.promise.settle(promiseByKey).then(promises => {
              if (
                promises.some(promise => promise.isRejected())
              ) {
                expect.fail({
                  diff(output) {
