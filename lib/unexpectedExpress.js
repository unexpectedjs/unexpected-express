        ...
        function doTheAssertions(errorPassedToNext) {
            ...
            try {
                ...
                var missingResponseProperties = Object.keys(expectedResponseProperties).filter(function (key) {
                    return !(key in httpResponse) && !(key in httpResponse.statusLine) && metadataPropertyNames.indexOf(key) === -1 && key !== 'url';
                });
                ...
                var missingRequestProperties = Object.keys(requestProperties);

                if (missingRequestProperties.length > 0) {
                    expect.fail('Property "' + missingRequestProperties[0] + '" does not exist on the request object.');
                }
