        ...
        expect
            ...
            httpRequest.headers.getNames().forEach(function (headerName) {
                if (headerName === 'set-cookie') {
                    headers[headerName] = [].concat(httpRequest.headers.valuesByName[headerName]);
                } else {
                    headers[headerName] = httpRequest.headers.valuesByName[headerName].join(', ');
                }
            });
...
console.log('req', req);
            ...
            if (typeof httpRequest.body !== 'undefined') {
                if (httpRequest.body.pipe) {
                    httpRequest.body.pipe(req);
                    if (httpRequest.body.constructor && httpRequest.body.constructor.name === 'FormData') {
                        if (!httpRequest.headers.has('Content-Type')) {
                            httpRequest.headers.set('Content-Type', 'multipart/form-data; boundary=' + httpRequest.body.getBoundary());
                    ...
                    httpRequest.body.on('data', function (chunk) {
            ...
            ['write', 'end', 'destroy'].forEach(function (methodName) {
                ...
                res[methodName] = function (chunk, encoding) {
                    ...
                    if (methodName !== 'destroy') {
                        ...
                        if (chunk && (!hasEnded || methodName === 'end')) {
