        ...
        function doTheAssertions(errorPassedToNext) {
            ...
            try {
                ...
                if (!httpResponse.satisfies(expectedResponseProperties)) {
                    var expectedHttpResponse = new messy.HttpResponse({
                        statusLine: _.defaults(_.pick(expectedResponseProperties, messy.StatusLine.propertyNames), httpResponse.statusLine),
                        headers: _.defaults({}, expectedResponseProperties.headers, httpResponse.headers.valuesByName),
                    ...
                    expect.fail(function (output) {
                        if (httpResponse.statusLine.satisfies(expectedHttpResponse.statusLine)) {
                            ...
                            output.append(expect.diff(expectedHttpResponse.statusLine.toString(), httpResponse.statusLine.toString()).diff);
                        }
                        output.nl();
                        if (httpResponse.headers.satisfy(expectedHttpResponse.headers)) {
                        ...
                        if (!('body' in expectedResponseProperties) || httpResponse.satisfies({body: expectedHttpResponse.body})) {
                            ...
                            output.append(expect.diff(expectedHttpResponse.body, httpResponse.body).diff);
                        }
                        return output;
                   });
                }
