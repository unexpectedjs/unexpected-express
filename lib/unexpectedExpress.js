...
var responsePropertyNames = messy.HttpResponse.propertyNames.concat(
  ...
  installInto: function(expect) {
    ...
    expect.exportAssertion(
      ...
      function(expect, subject, value) {
        ...
        var missingProperties = Object.keys(value).filter(function(key) {
          return key !== 'request' && key !== 'response';
        });
        if (missingProperties.length > 0) {
          throw new Error(
            'Property "' + missingProperties[0] + '" does not exist'
          );
        }
        ...
        var requestProperties =
          typeof value.request === 'string'
            ? { url: value.request }
            : _.extend({}, value.request);

        var requestBody = requestProperties.body;

        var requestForm = requestProperties.form;
        ...
        if (requestForm) {
          if (typeof requestForm !== 'string') {
            requestForm = qs.stringify(requestForm);
          }
        ...
        if (typeof requestProperties.query !== 'undefined') {
          if (
            ...
            var stringifiedQueryString = qs.stringify(requestProperties.query);
        ...
        expectedResponseProperties = _.omit(
          expectedResponseProperties,
          metadataPropertyNames
        );

        var missingResponseProperties = Object.keys(
          expectedResponseProperties
        ).filter(function(key) {
        ...
        if (missingResponseProperties.length > 0) {
          throw new Error(
            'Property "' +
              missingResponseProperties[0] +
              '" does not exist on the response object.'
          );
        }
        ...
        var matchProtocolVersion = String(httpRequest.protocolVersion).match(
          /^(\d+)(?:\.(\d+))$/
        );
        ...
        req.connection.remoteAddress =
          requestProperties.remoteAddress ||
          requestProperties.ip ||
          '127.0.0.1';
        ...
        var requestDestroyed = false;

        var done = false;

        var errorPassedToNext;
        ...
        return expect
          ...
          .then(function() {
            ...
            if (errorPassedToNext) {
              ...
              if (typeof expectedMetadata.errorPassedToNext !== 'undefined') {
                if (expectedMetadata.errorPassedToNext === true) {
                  ...
                  typeof expectedMetadata.errorPassedToNext === 'string'
                ) {
                  context.metadata.errorPassedToNext =
            ...
            return expect.promise.settle(promiseByKey).then(function(promises) {
              if (
                promises.some(function(promise) {
