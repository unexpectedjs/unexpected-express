...
function applyRequestBodyToToReq(requestBodyState, req, httpRequest) {
  if (requestBodyState) {
    const {
      hasFastJson,
      hasFormData,
    ...
    if (hasStream) {
      ...
      requestBody
        ...
        .on('end', () => {
          httpRequest.unchunkedBody = Buffer.concat(requestBodyChunks);
          req.push(null);
        })
        .on('error', err => {
          req.emit('error', err);
      ...
      // directly expose the input object which avoids all serialisation cost
...
class UnexpectedExpressMocker {
  constructor(subject, options) {
    ...
    options = options || {};
    this.useFastJson = !!options.useFastJson;
  }
  ...
  mock(options) {
    ...
    const { requestBody, requestBodyKind } = resolveRequestBody(
      requestProperties,
      httpRequest
    );
    switch (requestBodyKind) {
      case 'buffer':
      ...
      case 'unchunked':
        ...
        break;
      case 'raw':
        httpRequest.rawBody = requestBody;
        break;
      default:
    ...
    applyRequestBodyToToReq(requestBodyState, req, httpRequest);
